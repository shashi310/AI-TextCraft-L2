...
Text.post('/emotion',verify, async (req, res) => {
  try { 
      const { info } = req.body;
      // console.log(info, language );

      const response = await main4(info)
     //saving Data in history model
      let data= response[0].message.content
      // console.log("USER ID", req.body);
      const history = new HistoryModel({
...
async function main4(info) {
  const chatCompletion = await openai.chat.completions.create({
    messages: [
      {
        role: "system",
        content: "Certainly! Please provide the data you'd like me to detect emotion, and I'll provide the emotion based on the text"
      },  
      { role: 'user', 
      content: `I have some information and I would like you to detect the emotion based on that sentense,make sure you guess the releted emotion and here I am providing you the information \n ${info}` }],
...
// Text.post('/img', async (req, res) => {
//   try { 
//       const { prompt } = req.body||"a white siamese cat";
//       // console.log(info, language );

//       const response = await main4(prompt)
